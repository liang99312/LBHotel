...
function selectA01() {
    ...
    tj.paramters = a01;
    var options = {};
    options.url = "/LBTravel/a01/listA01sByPage.do";
    options.tj = tj;
    options.func = jxA01;
    options.ul = "#example";
...
function saveA01() {
    ...
    if (optFlag === 2) {
        ...
        a01 = a01s[editIndex];
        url = "/LBTravel/a01/updateA01.do";
    } else if (optFlag === 1) {
        url = "/LBTravel/a01/saveA01.do";
        a01.a01qx = "901;";
    }
    a01.bh = $("#inpBh").val();
    a01.mc = $("#inpMc").val();
...
function deleteA01(index) {
    ...
    if (confirm("确定删除员工：" + a01.mc + "?")) {
        $.ajax({
            url: "/LBTravel/a01/deleteA01.do?id=" + a01.id,
            contentType: "application/json",
            type: "get",
            dataType: "json",
            cache: false,
...
function saveA01Qx() {
    ...
    a01.a01qx = qx;
    $.ajax({
        url: "/LBTravel/a01/updateA01Qx.do",
        data: JSON.stringify(a01),
        contentType: "application/json",
        type: "post",
        cache: false,
